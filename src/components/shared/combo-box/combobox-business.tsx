...
import { fetchAllBusinessService } from "@/redux/features/master-data/store/thunks/business-thunks";

interface ComboboxSelectedProps {
  dataSelect: BusinessResponseModel | null;
  onChangeSelected: (item: BusinessResponseModel | null) => void;
  disabled?: boolean;
  label?: string;
  required?: boolean;
  size?: "sm" | "md" | "lg";
...
export function ComboboxSelectBusiness({
  ...
  const [data, setData] = useState<BusinessResponseModel[]>([]);
  ...
  const handleSelect = (item: BusinessResponseModel) => {
    if (item.id === "all") {
