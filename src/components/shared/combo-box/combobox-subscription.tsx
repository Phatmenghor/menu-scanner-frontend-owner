...
import { fetchAllSubscriptionService } from "@/redux/features/master-data/store/thunks/subscription-thunks";

interface ComboboxSelectedProps {
  dataSelect: SubscriptionResponseModel | null;
  onChangeSelected: (item: SubscriptionResponseModel | null) => void;
  disabled?: boolean;
  label?: string;
  required?: boolean;
  size?: "sm" | "md" | "lg";
...
const ALL_OPTION: SubscriptionResponseModel = {} as SubscriptionResponseModel;

export function ComboboxSelectSubscription({
  dataSelect,
  onChangeSelected,
  disabled = false,
  label = "Subscription",
  ...
  const [data, setData] = useState<SubscriptionResponseModel[]>([]);
  ...
  const fetchData = async (search: string, newPage: number) => {
    ...
    try {
      const result = await dispatch(
        fetchAllSubscriptionService({ search, pageNo: newPage, pageSize: 10 })
      ).unwrap();
  ...
  const handleSelect = (item: SubscriptionResponseModel) => {
    if (item.id === "all") {
    ...
    <div className="space-y-2 w-full">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {dataSelect
              ? dataSelect.businessName || dataSelect.planName || ""
              : placeholder}
                ...
                {data.map((item, index) => (
                  ...
                  >
                    ...
                    {item.id === "all" ? (
                      ...
                      <>{item.businessName || item.planName || ""}</>
                    )}
                  </CommandItem>
                ))}
              </CommandGroup>
