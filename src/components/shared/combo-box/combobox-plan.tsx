...
interface ComboboxSelectedProps {
  dataSelect: SubscriptionPlanResponseModel | null;
  onChangeSelected: (item: SubscriptionPlanResponseModel | null) => void;
  disabled?: boolean;
  label?: string;
  required?: boolean;
  size?: "sm" | "md" | "lg";
...
export function ComboboxSelectSubscriptionPlan({
  dataSelect,
  onChangeSelected,
  disabled = false,
  ...
  const [data, setData] = useState<SubscriptionPlanResponseModel[]>([]);
  ...
  const handleSelect = (item: SubscriptionPlanResponseModel) => {
    if (item.id === "all") {
